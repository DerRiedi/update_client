...
uint32_t CandidateApplications::getSlotForCandidate() {
  ...
  for (uint32_t slotIndex = 0; slotIndex < m_nbrOfSlots; slotIndex++) {    
    MbedApplication& mbedApplication = *m_candidateApplicationArray[slotIndex];
    if (! mbedApplication.isValid()) {
      return slotIndex;
    }
  }
  ...
  uint64_t oldestFirmwareVersion = m_candidateApplicationArray[oldestSlotIndex]->getFirmwareVersion();
  for (uint32_t slotIndex = 1; slotIndex < m_nbrOfSlots; slotIndex++) {    
    tr_debug(" Checking application at slot %d", slotIndex);
    if (m_candidateApplicationArray[slotIndex]->getFirmwareVersion() < oldestFirmwareVersion) {
      oldestSlotIndex = slotIndex;
      oldestFirmwareVersion = m_candidateApplicationArray[slotIndex]->getFirmwareVersion();
    }
  }
